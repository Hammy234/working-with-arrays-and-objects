...
function addTax (price){
  let afterTax = price * 1.07
  return afterTax
}

let postTaxPrices = prices.map( addTax );
...
function findSum (runningTotal, curElement){
  return runningTotal + curElement;
}

let totalPopulation = populations.reduce( findSum, 0)
...
function CPisOver200 (monster){
  return monster.CP > 200
}

let myStrongest = monstersInYourPocket.filter(CPisOver200)
...
function addSalesTax (order){
  return order.price + order.price * order.tax
}
let orderTotals = orders.map(addSalesTax)
...
/*
  Use a high order method to create to get the sum of bobs Total.
*/


function findTotal (runningTotal, purchase){
  if (purchase.owner === 'Bob'){
    return runningTotal + purchase.price
  } else {
    return runningTotal
  }
}

let bobsTotal = purchases.reduce(findTotal, 0)